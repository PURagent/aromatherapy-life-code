// Native Figma editor script. Uses only existing in-file Higgsfield image bytes.
...
async function buildAmberAtelier() {
  ...
  let page = figma.root.children.find(p => p.id === '0:1' && p.name === '01 · The Amber Atelier');
  if (!page) throw new Error('Expected Amber Atelier page was not found. Nothing changed.');
  await figma.setCurrentPageAsync(page);
  if(figma.root.children.some(p=>p.name==='02 · Native web design'))throw new Error('The native design page already exists. Stopped to avoid duplication.');
  ...
  page=figma.createPage();page.name='02 · Native web design';await figma.setCurrentPageAsync(page);
  ...
  // Existing draft artboards are preserved on their original page.
